...
/**
 * @task jekyll
 * Runs jekyll with environment-based
 * build arguments
 */
gulp.task("jekyll", cb => build(cb))
 ...
 * compiles the static site with Jekyll
 */
gulp.task("build", ["clean"], cb => {
  runsequence(["styles", "scripts", "svg"], "jekyll", cb)
})
...
/**
 * Execute Jekyll with Build Arguments based
 * upon environment variables
 * @param {Function} cb
 */
function build(cb) {
  const args = gulpConfig.jekyllArgs.default.concat(
    gulpConfig.jekyllArgs[argsType] || []
  )

  const generator = spawn("bundle", ["exec", "jekyll", "build"].concat(args), {stdio: "pipe", encoding: "utf-8"})

  generator.stdout.on("data", data => {
    log(null, data.toString(), "Jekyll")
  })
